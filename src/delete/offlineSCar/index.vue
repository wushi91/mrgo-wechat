<template>
  <div class="confirm-order">
    ...
    <div class="pay-wrapper">
      ...
      <div class="btn-pay" @click="makeOrder">去支付</div>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      shopCarFrids(goodList){
        let frids = []
        for(let i= 0;i<goodList.length;i++){
          frids = frids.concat(goodList[i].rfids)
        }
        return frids.join(',')
      },
      ...
      makeOrder(){
        let rfids = this.shopCarFrids(this.goodList)
        console.log('rfids',rfids)
        this.wxRequest.post.call(this,this.wxUrl.saveOrderByRfids,{needToken: true,rfids:rfids,sourceType:2,paymentType:4,isMember:2,userId:this.$store.getters.userInfo.id}).then(res=>{
          console.log('saveOrderByRfids res',res.data.content)
        },res=>{
          console.log('saveOrderByRfids err',res)
        })
